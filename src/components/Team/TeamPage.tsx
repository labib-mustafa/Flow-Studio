    ...
    <div className="flex-1 h-screen relative bg-[#f5f5f7] flex flex-col overflow-hidden text-left z-10">
      ...
      <header className="px-6 py-4 border-b border-slate-200/80 bg-white shrink-0 flex items-center justify-between gap-4 z-10 relative">
        <div className="flex items-center gap-2 w-1/3">
          <IdCardLanyard className="size-5 text-slate-900 shrink-0" />
      ...
      {/* Main Content Area */}
      <div className="flex-1 overflow-y-scroll px-8 py-6 custom-scrollbar bg-white">
                ...
                <>
                  <div className="relative">
                    <button
                      ...
                      className="flex items-center justify-between gap-2.5 pl-3.5 pr-2.5 bg-white border border-[#d1d5db] rounded-full text-xs text-[#111111] font-bold focus:outline-none focus:border-slate-800 transition-all cursor-pointer h-[38px] leading-none shrink-0"
                    >
          ...
          {activeTab === 'directory' && (
            <div>
              {filteredMembers.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 w-full justify-items-center">
                  {filteredMembers.map((member) => (
                    <motion.div
                      key={member.id}
                      layout
                      onClick={() => onSelectMember ? onSelectMember(member.id) : setSelectedMemberId(member.id)}
                      className="relative h-[460px] w-full max-w-[304px] mx-auto bg-[#B9B9B9] rounded-[20px] overflow-hidden group cursor-pointer text-left select-none font-plus-jakarta shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                    >
                      {/* Image Placeholder Icon (Top Center) or Profile Pic */}
                      {member.profilePic ? (
                        <div className="absolute inset-x-0 top-0 h-[316px] w-full overflow-hidden">
                          <img
                            src={member.profilePic}
                            alt={member.name}
                            className="w-full h-full object-cover origin-center"
                            style={{
                              transform: `scale(${member.profilePicScale ?? 1}) translate(${member.profilePicX ?? 0}px, ${member.profilePicY ?? 0}px)`
                            }}
                            referrerPolicy="no-referrer"
                          />
                        </div>
                      ) : (
                        <div className="absolute inset-x-0 top-0 h-[280px] flex items-center justify-center">
                          <Image className="w-12 h-12 text-[#9E9E9E]" />
                        </div>
                      )}

                      {/* Edit/Delete Actions overlay in top right */}
                      <div className="absolute top-4 right-4 z-20 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className="p-1.5 rounded-lg bg-white/90 text-slate-700 hover:text-slate-950 hover:bg-white shadow-sm transition-all"
                          title="Edit member"
                        >
                        ...
                        <button
                          ...
                          onClick={async (e) => {
                            ...
                            const ok = await confirm.danger(
                              `Delete ${member.name}?`,
                              'This member will be removed from the team directory.'
                            );
                            if (ok) {
                              deleteMember(member.id);
                            }
                          }}
                          className="p-1.5 rounded-lg bg-white/90 text-slate-700 hover:text-red-600 hover:bg-red-50 shadow-sm transition-all"
                          title="Delete member"
                        >
                      ...
                      {/* SVG at the bottom with clipped compass graphic */}
                      <svg className="absolute bottom-0 left-0 w-full h-[181px] overflow-hidden" viewBox="0 0 304 181" fill="none"
                        preserveAspectRatio="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <defs>
                          <clipPath id={`card-clip-${member.id}`}>
                        ...
                        <g clipPath={`url(#card-clip-${member.id})`}>
                          <image
                            ...
                            width="256"
                            height="256"
                            transform="translate(261, 79) rotate(28) scale(0.72) translate(-261, -79)"
                            style={{ opacity: 0.19, mixBlendMode: 'screen' }}
                          />
                        </g>
                      </svg>
                      ...
                      <div
                        className="absolute bottom-0 left-0 w-full h-[181px] px-[20px] py-[17px] flex flex-col justify-between z-10 text-white select-none">
                        ...
                        <div className="flex flex-col gap-[6px]">
                          ...
                          <p className="text-[12px] text-[white] opacity-[60%] font-normal line-clamp-1">{member.email}</p>
                          <div className="flex items-center gap-[5px] mt-2 text-[12px] text-slate-300">
                            <Folder className="w-3.5 h-3.5 text-white opacity-[60%]" />
                            <span className="text-[white] opacity-[60%] font-medium">Projects : {member.assignedProjects?.length || 0}</span>
                          </div>
                        </div>

                        {/* Bottom part: Title and ID */}
                        <div className="flex items-center justify-between text-xs font-semibold">
                          <span className="text-[#00F5FF] font-normal text-[12px]">{member.role}</span>
                          <span className="text-white font-normal text-[12px]">ID: Token_${member.id.replace('m_', '').substring(0, 8)}</span>
                        </div>
                      </div>
                    </motion.div>
                  ))}
                          ...
                          >
                            <td className="py-4 px-6">
                              <div className="flex items-center gap-3">
                                {member.profilePic ? (
                                  <div className="w-9 h-9 rounded-full overflow-hidden border border-[#e5e7eb] shrink-0">
                                    <img
                                      src={member.profilePic}
                                      alt={member.name}
                                      className="w-full h-full object-cover origin-center"
                ...
                <div className="flex flex-col items-center text-center pb-6 border-b border-[#e5e7eb]">
                  <div className="relative mb-4 group">
                    {selectedMember.profilePic ? (
                      <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-[#e5e7eb] shadow-sm shrink-0">
                        <img
                          src={selectedMember.profilePic}
                          alt={selectedMember.name}
                          className="w-full h-full object-cover origin-center"
                          style={{
                            transform: `scale(${selectedMember.profilePicScale ?? 1}) translate(${selectedMember.profilePicX ?? 0}px, ${selectedMember.profilePicY ?? 0}px)`
                          }}
                        />
                      </div>
                    ) : (
              ...
              <div className="p-6 border-t border-[#e5e7eb] bg-[#f9fafb] flex items-center justify-between gap-3">
                <button
                  ...
                  onClick={async () => {
                    const ok = await confirm.danger(
                      'Remove Team Member?',
                      'Are you sure you want to delete this member from the workspace?'
                    );
                    if (ok) {
