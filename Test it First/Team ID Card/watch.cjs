...
http.createServer((req, res) => {
  ...
  if (req.url.startsWith('/open-code')) {
    const urlObj = new URL(req.url, 'http://localhost:35731');
  ...
  if (req.url === '/save' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        console.log('ID Cards code saved successfully!');
  ...
  console.log('Hot reload & save server running on http://localhost:35731');
});
